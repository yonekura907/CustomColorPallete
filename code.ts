...
const BRAND_PRIMITIVE: ColorItem[] = [
...
const BRAND_USER_LIGHT: ColorItem[] = [
...
function solidPaint(rgb: RGB): SolidPaint {
  return { type: "SOLID", color: rgb };
}

// 選択中ノードに色を適用
function applyColor(hex: string, target: "fill" | "stroke" | "auto"): number {
  ...
  const paint: SolidPaint = solidPaint(rgb);
  let applied = 0;
  ...
  for (const node of selection) {
    try {
      ...
      if ("fills" in node) {
        const fillable = node as MinimalFillsMixin;
...
async function main(): Promise<void> {
  ...
  try {
    saved = (await figma.clientStorage.getAsync(STORAGE_KEY)) as
      | Palette[]
      | undefined;
  } catch (_e) {
    saved = undefined;
...
function mergeWithDefaults(saved: Palette[]): Palette[] {
